    ...
    <div className="min-h-screen bg-black text-white pt-40 pb-32 px-6 md:px-12">
      <div className="max-w-[1200px] mx-auto space-y-24">
        ...
        <header className="space-y-6">
           <h1 className="text-6xl md:text-8xl font-display font-black tracking-tighter uppercase italic">DEPLOYS</h1>
           <p className="text-white/20 uppercase tracking-[0.4em] text-[10px] font-black">{orders.length} ACTIVE TRANSMISSIONS</p>
        </header>
          ...
          <div className="space-y-12">
            {orders.map((order) => (
              <motion.div 
                ...
                className="p-10 md:p-16 rounded-[4rem] bg-[#0A0A0A] border border-white/5 space-y-12 relative group hover:border-brand-red/30 transition-all duration-700"
              >
                 <div className="flex flex-col md:flex-row justify-between gap-8 border-b border-white/5 pb-12">
                    <div className="space-y-4">
                       <p className="text-[10px] uppercase font-black tracking-widest text-white/40">Transmission ID</p>
                       <p className="text-2xl font-mono text-white/60">#{order.id.toUpperCase()}</p>
                    </div>
                    <div className="flex gap-12">
                       <div className="space-y-1">
                          <p className="text-[10px] uppercase font-black tracking-widest text-white/40">Sync Date</p>
                          <p className="text-sm font-bold">{order.createdAt?.toDate().toLocaleDateString()}</p>
                       </div>
                       <div className="space-y-1">
                          <p className="text-[10px] uppercase font-black tracking-widest text-white/40">Total Value</p>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex gap-6 items-center">
                         <div className="w-20 h-24 rounded-2xl bg-white/5 overflow-hidden flex-shrink-0 group-hover:grayscale-0 grayscale transition-all duration-700 border border-white/5">
                            <img src={item.imageUrl} className="w-full h-full object-cover" />
                         </div>
                         <div className="space-y-1">
                            <p className="text-xs font-black uppercase tracking-tight truncate w-32">{item.name}</p>
                            <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">Qty: {item.quantity} | {item.size}</p>
                         </div>
                      </div>
                    ))}
                 </div>
                 ...
                 <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-8 border-t border-white/5">
                    <p className="text-[10px] uppercase font-black tracking-[0.4em] text-white/10 italic">Secured deployment via WH1RLPOOL protocols</p>
                    <div className="flex gap-4">
                       <Button variant="outline" className="rounded-full px-8 h-12 text-[10px] font-black border-white/10 hover:bg-white hover:text-black">TRACK DEPLOYMENT</Button>
                    </div>
                 </div>
              </motion.div>
            ))}
